        ...
        syntaxTree(view.state).iterate({
            ...
            enter: (node) => {
                ...
                switch (node.name) {
                    case "ListMark": {
                        if (!isInSelection) {
                            decorations.push(Decoration.mark({ class: 'rtm-listMark' })
                                .range(node.from, node.to));
                        }
                        else {
                            decorations.push(Decoration.mark({ class: "rtm-listMarkSelected" })
                                .range(node.from, node.to));
                        }
                        break;
                    }
                    case "ListItem": {
                        decorations.push(Decoration.mark({ class: "rtm-listItem" }).range(node.from, node.to));
                        break;
                    }
                    case "QuoteMark": {
                        if (!isInSelection) {
                            decorations.push(Decoration.mark({ class: 'rtm-quoteMark' }).range(node.from, node.to));
                        }
                        else {
                            decorations.push(Decoration.mark({ class: "rtm-quoteMarkSelected" }).range(node.from, node.to));
                        }
                        break;
                    }
                    case "Blockquote": {
                    ...
                    case "StrongEmphasis":
                        if (!isInSelection) {
                            decorations.push(Decoration.replace({}).range(node.from, node.from + 2), Decoration.replace({}).range(node.to - 2, node.to));
                        }
                        decorations.push(Decoration.mark({ class: "rtm-strongEmphasis" }).range(node.from + 2, node.to - 2));
                        break;
                    case "Emphasis":
                        if (!isInSelection) {
                            decorations.push(Decoration.replace({}).range(node.from, node.from + 1), Decoration.replace({}).range(node.to - 1, node.to));
                        }
                        decorations.push(Decoration.mark({ class: "rtm-emphasis" }).range(node.from + 1, node.to - 1));
                        break;
                    case "Link":
                    case "LinkMark": {
                        ...
                        if (match) {
                            ...
                            if (!isInSelection && url) {
                                ...
                                decorations.push(Decoration.mark({ class: "rtm-link" }).range(node.from, node.to));
                            }
                        }
                        break;
                    }
                    case "URL": {
                        if (isInSelection) {
                            if (isModKeyPressed) {
                                ...
                                decorations.push(Decoration.mark({ class: "rtm-url" }).range(node.from, node.to));
                            }
                        }
                        else {
                            if (!isModKeyPressed) {
                    ...
                    case "ATXHeading6": {
                        ...
                        if (!isInSelection && text) {
                            ...
                            decorations.push(Decoration.mark({ class: "rtm-headerMark" }).range(node.from, node.from + prefixLength));
                        }
